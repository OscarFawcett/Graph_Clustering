...
Usually, this data will be given in what is known as an adjacency matrix. The adjacency matrix $A$ is an $n \times n$ matrix, where $n$ represents the number of nodes in the graph. Element $(i,j) \in A$ equals 1 if an edge exists between nodes $i$ and $j$, and 0 otherwise. 
